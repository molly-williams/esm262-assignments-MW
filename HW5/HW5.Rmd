...
### Background
We chose to develop our package based on ENSO data, as climate change is expected to intensify impacts from El Nino and La Nina. We created two functions to summarize the monthly ENSO data from 1950-2018 (which ranges from -2 to 2 and is based on sea surface temperature monitoring) and three functions to estimate weather impacts and costs from increased risk of environmental disturbances. 
...
2) **Impact calculation functions**
    - predict_precip(): predict precipitation in a specific location based on ONI value using a linear model. lm under the hood, in function user inputs index score, output will be rain fall. Rainfall ~ ONI (user will input value between -2 and 2)
...
enso_data <-  enso_data_raw %>% 
  ...
  mutate(date = with(enso_data, sprintf("%d-%02d", year, month)))
