...
computediversity = function(species, Alternate=FALSE) {
...
if (Alternate) {
  tmp = sum(summary(species)*(summary(species)-1))/
  (sum(summary(species))* (sum(summary(species))-1))
diversity = 1.0-sum(tmp)
}
...
return(list(num=nspecies, simpson=diversity, rare=rare))
}
