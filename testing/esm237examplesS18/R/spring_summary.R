...
spring_summary = function(clim_data, spring_months = c(4:6), springout = FALSE) {
  ...
  if (springout)
    return(
      list(
        mean_springT      = mean_springT,
        coldest_spring_yr = lowyear,
        coldest_springT   = lowT,
        mean_springP      = mean_spring_precip,
        wettest_springP   = wettest_spring,
        wettest_spring_yr = wettest_spring_yr,
        all.springP       = spring_precip,
        all.springT       = all.springT
      )
    )
  else
    return(
